...
class ChooseModeView(View):
    ...
    def get(self, request, course_id, error=None):  # lint-amnesty, pylint: disable=too-many-statements
        ...
        if waffle.switch_is_active('local_currency'):
            if 'edx-price-l10n' not in request.COOKIES:
                ...
                LOG.info('[Track Selection Check] Currency data: [%s]', currency_data)
                try:
    ...
    def post(self, request, course_id):
        ...
        if not user.has_perm(ENROLL_IN_COURSE, course):
            ...
            LOG.info('[Track Selection Check] Error: [%s]', error_msg)
        ...
        if requested_mode not in allowed_modes:
            LOG.info('[Track Selection Check] Error: requested enrollment mode [%s] is not supported', requested_mode)
        ...
        if requested_mode == 'verified':
            ...
            LOG.info(
                '[Track Selection Check] Requested verified mode - contribution: [%s], contribution other amount: [%s]',
                request.POST.get("contribution"), request.POST.get("contribution-other-amt")
            )
            ...
            try:
                ...
                LOG.info('[Track Selection Check] Requested verified mode - Error: [%s]', error_msg)
                return self.get(request, course_id, error=error_msg)
            ...
            if amount_value < mode_info.min_price:
                ...
                LOG.info(
                    '[Track Selection Check] Requested verified mode - Error: amount value [%s]'
                    'is less than minimum price [%s]',
                    amount_value, mode_info.min_price
                )
                return self.get(request, course_id, error=error_msg)

            donation_for_course = request.session.get("donation_for_course", {})
            LOG.info(
                '[Track Selection Check] Donation for course: [%s], amount value: [%s]',
                donation_for_course, amount_value
            )
